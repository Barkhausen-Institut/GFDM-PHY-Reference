...
from modem.util import ChannelEstimation, Equalizer
from modem import Modem, get_ofdm_preamble_based
...
class TestEqualizer(unittest.TestCase):
    ...
    def test_channel_equalizer(self):
        ...
        equalizer = Equalizer()
        eq_payload = np.fft.ifft(equalizer.equalize(
            np.fft.fft(rx_payload), estimated_channel))
