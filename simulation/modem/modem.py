...
from modem.util import FrozenBoundedClass
sys.path.append(os.path.join(os.path.dirname(__file__), '..','..'))
import sdr_utils.vector as vec
import sdr_utils
...
class Modem:
    def __init__(self, param):
        self.param = param
        self._qam4_mapper = util.QamMapper(4)
        self._qam16_mapper = util.QamMapper(16)
        self._qam64_mapper = util.QamMapper(64)
        self.intern_data = {}
    ...
    def _run_chain(self, data, structure):
        ...
        for block in structure:
            ...
            data_in = data_out
            if self.save_intern_data: self.intern_data[block] = data_out
        return data_out
    ...
    def bytes_per_frame(self):
        ...
        else: raise ValueError(f"invalid number of bits: {self.bits_per_frame}")

    @property
    def bits_per_frame(self):
        # payload bits ber frame 
        return int(self.symbols_per_frame * np.log2(self.param.qam_order)  / self._coder.code_rate)
    ...
    @property 
    def _coder(self):
        return util.Coder(type_=self.param.coder_type)
    ...
    def _fullpreamble(self):
        return np.concatenate((self.param.halfpreamble,self.param.halfpreamble))

    @property
    def _fullpreamble_cpcs(self):
        return util.add_cp_cs(self._fullpreamble, self.param.Ncp, self.param.Ncs)
    @property
    def _windowing(self): 
        return util.Windowing(self.param.Ncp,self.param.Ncs,self.param.windowing_alpha)

    def frame_multiplexer(self, data):
        fmux = util.FrameMultiplexer()
        return fmux.multiplex_frame(
            self._windowing.apply_window(data), self._windowing.apply_window(self._fullpreamble_cpcs) 
            )
        
    #--------------- CYCLIC PREFIX/SUFFIX -------------------#
    def add_cp(self, data):
        return util.add_cp(data, self.param.Ncp)

    def remove_cp(self, data):
        return util.remove_cp(data, self.param.Ncp)

    def add_cs(self, data):
        return util.add_cs(data, self.param.Ncs)

    def remove_cs(self, data):
        return util.remove_cs(data, self.param.Ncs)
    
    def add_cp_cs(self,data):
        return np.concatenate(
            [util.add_cp_cs(data[b], self.param.Ncp, self.param.Ncs) for b in range(self.param.B)]
        )
        #return util.add_cp_cs(data, self.param.Ncp, self.param.Ncs)
    ...
    @property
    def _modulator(self):
        return util.Modulator(self.param.K,self.param.M, self.param.modulator_type)
        
    def modulator(self, data):
        data_out = np.empty((self.param.B,self.param.K*self.param.M), dtype = data.dtype)
        for b in range(self.param.B):
            if self.param.modulator_type in ("OFDM","ofdm"):
                data_out[b] = sdr_utils.norm_complex(self._modulator.modulate(data[b,:,0]))
            elif self.param.modulator_type in ("GFDM","gfdm"):
                data_out[b] = sdr_utils.norm_complex(self._modulator.modulate(data[b]))
        return data_out
    ...
    def _synchronization(self):
        return util.synchronization(self.param)
    ...
    def sync(self, data):
        ...
        if len(preamble_starts) == 0:
            return np.array([])
        preamble_start = preamble_starts[0] # take only first detected frame (discard other)  
        if self.save_intern_data:
        ...
        try:
            preamble = data[preamble_start:preamble_start+self.param.Npreamble]
            idx = np.arange(self.param.Npayload)
            payload = np.vstack(
                [data[(preamble_start + Npre_cp_cs + Npay_cp_cs*b) + idx] for b in range(self.param.B)]
            )
    ...
    def _channel_est_eq_preamble(self, data):
        ...
        estimator = util.ChannelEstimation()
        freq_response = estimator.estimate_channel_form_preamble(received_preamble, self.param.halfpreamble)
        equalizer = util.Equalizer()

        data_out = np.vstack(
            [equalizer.equalize(np.fft.fft(received_payload[b]), freq_response) for b in range(self.param.B)]
        )     
    ...
    def _channel_est_eq_pilot(self,data):
        ...
        demapped_data = self.resource_demapper(np.resize(payload,(self.param.B, self.param.K, self.param.M)))
        estimator = util.ChannelEstimation()
        equalizer = util.Equalizer()
        return np.vstack(
            [equalizer.equalize(payload[b], 
                                estimator.estimate_channel_from_pilots(demapped_data[b][1], self.param.pilots) )
                for b in range(self.param.B)]
        ) 
    ...
    def _resource_mapper(self):
        return util.ResourceMapper(self.param.K, self.param.M, self.param.Kset, self.param.Mset, self.param.pilot_pos)
    
    def resource_mapper(self, data):
        data_out = np.empty((self.param.B,self.param.K,self.param.M), dtype = type(data[0]))
        data_resh = np.reshape(data,(self.param.B,-1))
        for b in range(self.param.B):
            data_out[b] = self._resource_mapper.mapper(data_resh[b], self.param.pilots)
        return data_out
